...
### Art / Assets
- [ ] **Create images for the Artifacts** — Power cards, champion cards, and other game artifacts need proper artwork

### Multiplayer
- [ ] **E2E multiplayer testing** — Draft works, need full game flow (place workers, shops, round transitions)
- [x] **Guest game view** — Fixed: null pendingDecision between draft picks no longer shows blank screen; WaitingOverlay stays visible during opponent decisions (was flickering off)
- [x] **Double advanceRound race** — Added phase guard in doAdvanceRound to prevent both players triggering round advance

### Balance
- [ ] **Cash In T3 redesign** — Current design is boring (no decisions), enables gold hoarding dominance
- [x] **Golden Scepter double-fire** — Investigated: cannot reproduce. Architecture prevents it (resource re-snapshot line 274, triggerOn:'self', skipHandlerIds). Likely fixed by ship sprint changes.
- [x] **doubleNextTheft orphaned** — Fixed: black strong shop effect now consumed by all steal actions (ransack, pickpocket, plunder, tribute) + black_weak shop. Doubles steal amounts.
- [x] **Chrono Compass doesn't update turn order** — Fixed: resolveDecision now sets currentPlayer = turnOrder[0] after Chrono Compass reorders turn order.
- [ ] **Rainbow Scepter pendingDecision lost in eternity chain** — Decision gets swallowed when eternity replays an action that triggers Rainbow Scepter
...
- [x] **Fix multiplayer draft** — HostSync null guard prevents rejecting valid draft actions
- [x] **Animation overhaul (8 features)** — Turn tab pulse, AI thinking dots, action space flash, board turn flash, board dim during watching, victim tab flash, active effects banner, market sold flash
- [x] **Rejoin interstitial** — Replaced auto-rejoin with Rejoin/Leave prompt (works during draft too)
- [x] **AI action name display** — TurnIndicator shows "→ Action Name" after AI/remote player places
- [x] **Generalized watching** — All visual feedback works for both AI and remote human opponents
- [x] **Louder opponent action feedback** — Placement banner ("P1 → Action") + resource drip pills float from action spaces
- [x] **Stronger placeFlash** — 400ms→800ms, added outer glow alongside inset glow
